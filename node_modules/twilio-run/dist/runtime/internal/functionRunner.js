...
const handleError = (err) => {
    if (err) {
        process.send && process.send({ err: serialize_error_1.serializeError(err) });
    }
};
const handleSuccess = (responseObject) => {
    ...
    if (typeof responseObject === 'string') {
        ...
        route_1.isTwiml(responseObject)) {
...
process.on('message', ({ functionPath, event, config, path }) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        yield route_cache_1.getRouteMap(config);
        route_1.constructGlobalScope(config);
        const context = route_1.constructContext(config, path);
        ...
        if (process.send) {
            process.send({ err: serialize_error_1.serializeError(err) });
        }
    }
}));
